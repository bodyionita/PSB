...
stores), runs one job, and tears down. The M2 combined ``reindex`` (git pull → rescan → recompute
graph → commit+push) is here too — handy for the "DB wipe + reindex restores search" recovery
drill without going through the authenticated API.
...
# The M2 combined reindex (ADR-023 §4) is driven by its own service, not BackupJobs.
REINDEX = "reindex"
...
async def run_job(name: str) -> None:
    ...
    try:
        ...
        if name == REINDEX:
            await build_reindex_service(settings, db, vault_backup).run_scheduled()
        else:
