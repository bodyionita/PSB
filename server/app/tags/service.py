...
Propose and apply are deliberately split so the human reviews the plan before any store write:
  ...
  * :meth:`apply` — takes a (reviewed) plan back, opens an ``agent="tags-consolidate"`` run, and
    rewrites the affected nodes' ``tags:`` frontmatter + reindexes them in the **background**, so
    the endpoint answers ``202 {run_id}`` (03-api §Admin). Reuses the same never-lose treatment as
    the reorganize path: atomic writes, git-tracked + revertible, skip-and-continue per node.
...
Consolidation changes only frontmatter tags, not node bodies, so a touched node re-embeds to the
same vectors — the derived edges are untouched (recomputed nightly / on reindex, ADR-023).
"""
...
class StoreCommitter(Protocol):
    """The single store-git op apply needs: a forced commit + push of the rewritten tags."""
...
class TagConsolidationService:
    ...
    async def propose(self) -> ConsolidationProposal:
        ...
        """
        counts = await self._store.tag_counts(limit=self._settings.tags_consolidate_max_vocabulary)
    ...
    async def _run_apply(self, run_id: str, mapping: dict[str, str]) -> None:
        """Rewrite every affected node's ``tags:`` line, reindex the changed ones, commit+push.

        Never raises (rule 7): a per-node read/write error is logged and skipped (skip-and-continue,
        mirroring the indexer), and any unexpected failure ends the run ``failed`` with context.
        The graph store is truth (rule 1), so a partial apply is safe to re-drive.
        """
        try:
            variants = sorted(mapping.keys())
            affected = await self._store.nodes_with_any_tag(variants) if variants else []
            changed: list[str] = []
            failed = 0
            for node in affected:
                try:
                    if await self._rewrite_one(node.store_path, mapping):
                        changed.append(node.store_path)
                except Exception:  # noqa: BLE001 — one bad node must not abort the apply (rule 7)
                    logger.exception(
                        "tags consolidate: failed to rewrite %s, skipping", node.store_path
                    )
                    failed += 1
            ...
            summary = (
                f"tags consolidate: {len(changed)} node(s) rewritten across "
                f"{len(mapping)} variant(s) → {len(set(mapping.values()))} canonical tag(s)"
            )
            if failed:
                summary += f", {failed} skipped"
            if index.partial:
                # An embed skip left a rewritten node transiently stale in the index; the store is
            ...
            await self._runs.finish(
                ...
                details={
                    "mapping": mapping,
                    "nodes_rewritten": len(changed),
                    "nodes_skipped": failed,
                    "index": index.as_dict(),
    ...
    async def _rewrite_one(self, store_path: str, mapping: dict[str, str]) -> bool:
        """Rewrite one node's ``tags:`` line if any variant is present; return True when written."""
        raw = await asyncio.to_thread(self._read, store_path)
        if raw is None:
            logger.warning("tags consolidate: node %s missing on disk, skipping", store_path)
            return False
        rewritten, changed = rewrite_node_tags(raw, mapping)
        if not changed:
            return False
        await asyncio.to_thread(self._write, store_path, rewritten)
        return True
    ...
    # --- store filesystem (atomic, ADR-014; mirrors the node writer) -------------------------

    def _read(self, store_path: str) -> str | None:
    ...
    def _write(self, store_path: str, contents: str) -> None:
        path = self._root / Path(*store_path.split("/"))
